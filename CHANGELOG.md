...
### Added
- **`devils-advocate` agent:** a new, **optional** agent offered in `/teamme:init-team`'s Phase 3 questionnaire and declinable. It reads and asks questions of the designs the lanes return for an approved brief — what was rejected, why, what assumptions carry risk — and dispatches nothing. Tools: `Read`, `Grep`, `Glob`, plus the work-log MCP tool. No `Bash`, no `Task`, no `Edit`, no `Write`. Lives in a new **third agent tier**, `plugins/teamme/templates/agents/`, shipping project-agnostic and copied verbatim when selected, per the copy-don't-re-author invariant. Teamme previously had two: always-present-shipped (`history-librarian`, in every project from day one, not roster-selectable) and generated-per-project (derived from that project's layout). The advocate is neither — identical everywhere but optionally declined, so it is shipped and declinable together.
- **Design-return cycle in `/intake`:** between dispatch and implementation, each lane now returns a **design** — the approach, what was rejected and why, assumptions and uncertainties — before any code is written. The orchestrator runs the advocate over every design and relays its questions back to the lanes. **The orchestrator alone decides** what to action, overrule or fork to the user. Two invocation modes: **per-lane** (one design at a time, with that lane's framing mounted) and **cross-lane** (every design at once, spotting divergences like two independent solutions to the same problem choosing different technologies). A single-lane brief runs per-lane only. A **single trigger rule** gates both steps: engage when the brief adds a mechanism that did not exist, touches a design invariant, spans more than one lane, or reverses a prior decision; skip when it is already-specified, single-lane, no new mechanism, no invariant in reach. Intake says in one line which branch it took. `/teamme:queue` and deferral short-circuits never reach this step, so parking a request stays free.

### Fixed
- **Shipped prompts were silently unvalidated.** `validate.sh`'s manifests check excluded `templates/` **by path component** — a rule that was exact when `templates/` held only one `.md` file, the `{{PLACEHOLDER}}` skeleton `intake.md`. The check's own comment read: "A third prompt directory added later fails this loudly instead of silently passing unchecked." That promise was never tested — no assertion had ever added a third prompt directory and confirmed rejection. When `templates/agents/devils-advocate.md` shipped, the rule silently exempted it: a real agent prompt with real frontmatter, shipped entirely unvalidated. Fixed by replacing the path exclusion with a **content property**: a file is exempt only if it contains a `{{PLACEHOLDER}}`-shaped marker, so a future skeleton is exempt automatically with no path to add, and a future non-skeleton under `templates/` is never exempt by location alone. A new encoded watch-fail asserts the walk flags a rogue unaccounted file. `templates/agents/*.md` is now validated exactly like `agents/*.md` — frontmatter parses, name and description required, `permissionMode`/`hooks`/`mcpServers` rejected — through a shared `check_agent_file()`.

### Known limitations
- **No eval suite for any of this.** `devils-advocate`'s prompt, the design-return cycle, and the trigger rule are all instruction, not enforcement. Nothing tests whether the advocate asks useful questions, whether it returns "no objection" when it should, or whether it terminates; nothing blocks an orchestrator that skips the cycle entirely. Same register as every other convention teamme states rather than enforces.
- **The watch-fail count in this repo's docs was an undercount and is now stated as a floor.** `grep -c 'echo "== .*\[watch-fail\]'` over `scripts/validate.sh` returns 19, counting sections whose *header* carries the tag; six further sections encode a watch-fail without one, so 25 of 96 sections encode at least one. A single section can encode several independent watch-fails (e.g. a co-change damping cap is break-tested four ways in one section), so the count of individual watch-fails is higher still, and no available grep yields it.
- **`README.md` is still excluded from the prompt walk by *name*, tree-wide**, correct today only because every `README.md` in the tree happens to carry no frontmatter. The failure direction is safer (a doc named anything else is flagged loudly), and it is recorded rather than closed.
...
### Fixed
- `_install_evidence()` in `preflight.py` was testing one thing and reporting another: it checked for the mere existence of `.claude/commands/intake.md` but claimed it was generated by teamme. **"intake" is not a teamme-only word**, so a project carrying its own unrelated `.claude/commands/intake.md` was misidentified as an `installed-outdated` teamme install — and pointing it at a repair that writes teamme scaffolding into a repo that never asked for it. The probe now requires the file to name one of teamme's own hook scripts via `_text_names_a_teamme_hook()`, reading at most 1 MiB. A project with a coincidentally-named `intake.md` now correctly scores as `not-installed` instead. No real teamme install is affected: a generated `/intake` command names teamme's scripts by construction (measured: 22-27 references spanning at least two distinct scripts across every version of the template in this repo's history). **`any()` not `all()`:** checking for *any* required hook avoids regressing older installs that predate a newer hook — a release that adds a hook only adds another way to match, so an older install can never age out of matching.

### Known limitations
- An `intake.md` larger than 1 MiB whose first teamme script reference falls past the byte cap contributes no evidence. `validate.sh` pins this to the exact byte with a fixture of exactly `1 << 20` padding bytes followed immediately by `worklog.py`, and the probe genuinely misses it. This cannot break a real install alone: the settings-hooks-block probe is independent, so both the file content *and* the settings hooks would have to be missing for an actual teamme install to regress to `not-installed`.
